...
class Config:
    ...
    def __init__(self, base_path: str, override_path: str, key: str) -> None:
        ...
        converted_dict["meta"].update(
            {"timestamp": timestamp, "key": f"{timestamp}_{key}" if key else timestamp}
        )

        self._config: dict = converted_dict
