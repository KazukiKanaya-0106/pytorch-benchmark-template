...
class BinaryFocalLoss(nn.Module):
    ...
    def forward(
        self, predicted_probs: torch.Tensor, collect_labels: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        """
        # 正解ラベルの確率 (B, 1, H, W)
        correct_label_probs = collect_labels * predicted_probs + (
            1 - collect_labels
        ) * (1 - predicted_probs)
        ...
        smoothing = 1e-8
        correct_label_probs = torch.clamp(
            correct_label_probs, smoothing, 1.0 - smoothing
        )

        # フォーカル損失の計算
        loss = (
            -alpha_t
            * ((1 - correct_label_probs) ** self.gamma)
            * torch.log(correct_label_probs)
        )
